...
export default function OrderTicket({ onCheckout }) {
  const { currentOrder, subtotal, total, clearCart, exchangeRate } = useApp();
  const { items, orderType, paymentStatus, advanceAmount } = currentOrder;
    ...
    <div className="flex flex-col h-full bg-slate-50 flex-1">
      ...
      <div className="flex items-center justify-between px-4 py-3 bg-white border-b border-slate-100 shrink-0">
        <div className="flex items-center gap-2 min-w-0 flex-1">
          <Receipt className="w-5 h-5 text-pizza-red shrink-0" />
          <h2 className="text-slate-800 font-bold text-sm shrink-0">Ticket</h2>
          {items.length > 0 && (
            <span className="bg-pizza-red text-white text-[10px] font-bold px-1.5 py-0.5 rounded-full shrink-0">
              {items.length}
            </span>
          )}
          {/* Badge del tipo de pedido */}
          {typeConfig && (
            <span className={`inline-flex items-center gap-1 px-2 py-0.5 rounded-full text-[10px] font-bold border ${typeConfig.color} shrink-0 ml-1`}>
      ...
      {paymentLabel && (
        <div className={`px-4 py-1.5 flex items-center gap-2 text-xs font-semibold border-b border-slate-100 ${paymentLabel.color}`}>
