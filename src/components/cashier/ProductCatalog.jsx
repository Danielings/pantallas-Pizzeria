    ...
    <div className="modal-backdrop" onClick={onClose}>
      <div className="modal-content p-6 w-80" onClick={e => e.stopPropagation()}>
        ...
        <div className="flex gap-2">
          ...
          <button onClick={() => onConfirm(selected)} className="btn-primary flex-1">Agregar</button>
        </div>
      </div>
    </div>
  );
...
function ProductCard({ product }) {
  ...
  return (
    <>
      ...
      >
        {product.image ? (
          <img src={product.image} alt={product.name} className="w-full h-32 object-cover rounded-lg mb-1 shadow-sm" />
        ) : (
          <div className="text-3xl mb-1 text-center">{product.emoji}</div>
        )}
        <p className="text-pizza-dark font-semibold text-sm leading-tight line-clamp-2">{product.name}</p>
        <p className="text-pizza-muted text-xs line-clamp-2">{product.description}</p>
        {product.sizes?.length > 0 && (
          <p className="text-pizza-muted text-xs">{product.sizes.join(' · ')}</p>
        )}
        <div className="flex items-center justify-between mt-auto pt-2">
          <span className="text-pizza-red font-bold text-base">${product.price.toFixed(2)}</span>
      ...
      {showSizeModal && (
        <SizeModal product={product} onConfirm={handleConfirmSize} onClose={() => setShowSizeModal(false)} />
      )}
    </>
  );
}
      ...
      {isSearching && (
        <div className="px-4 pt-3 pb-1">
          <p className="text-pizza-muted text-sm">
            {displayProducts.length} resultado{displayProducts.length !== 1 ? 's' : ''} para <span className="text-pizza-dark font-semibold">"{search}"</span>
          </p>
        </div>
      )}
