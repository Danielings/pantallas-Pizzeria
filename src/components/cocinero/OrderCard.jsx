...
export function OrderCard({ order }) {
  ...
  return (
    <div className={`bg-white rounded-xl border border-pizza-gray-3 border-l-4 ${cfg.border} p-4 flex flex-col gap-3 animate-card-move shadow-card`}>
      {/* Header */}
      ...
      <div className="flex flex-col gap-1.5">
        {order.items.map((item, i) => (
          <div key={i} className="flex items-start gap-2 bg-pizza-gray-2 border border-pizza-gray-3 rounded-lg px-3 py-2">
            <span className="text-pizza-red font-bold text-sm w-5 flex-shrink-0">{item.qty}×</span>
            <div className="min-w-0">
              <p className="text-pizza-dark text-sm font-medium leading-tight">{item.name}</p>
              {item.size && (
                <p className="text-pizza-muted text-xs">{item.size}</p>
              )}
              {item.extras && item.extras.length > 0 && (
                <p className="text-pizza-red text-xs mt-0.5 font-medium">+ {item.extras.map(e => e.name).join(', ')}</p>
              )}
            </div>
          </div>
        ))}
      ...
      <div className="flex justify-between items-center text-xs text-pizza-muted pt-1 border-t border-pizza-gray-3">
        ...
        <span className="text-pizza-dark font-semibold">${order.total.toFixed(2)}</span>
      </div>
