...
export const obtenerResumenDia = async (req, res) => {
  try {
    ...
    // 3. Ticket promedio
    const ticket_promedio = total_ordenes > 0 ? (ventas_totales / total_ordenes) : 0;
    ...
    const punto_de_venta_en_usd = tasa_cambio > 0 ? punto_de_venta_bs / tasa_cambio : 0;
    const transferencia_en_usd = tasa_cambio > 0 ? transferencia_bs / tasa_cambio : 0;

    // Total general en divisa (USDT) — suma todos los canales convertidos
    const total_divisa = Number(
      (efectivo_usd + efectivo_bs_en_usd + punto_de_venta_en_usd + transferencia_en_usd).toFixed(2)
    );
